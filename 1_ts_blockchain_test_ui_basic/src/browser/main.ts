...
(async function main(): Promise<void> {
  ...
  const gont_blockchain = new Blockchain();
  await gont_blockchain.createGenesisBlock();
  blocksEl.innerHTML = gont_blockchain.chain.map((b, i) => generateBlockHtml(b, i)).join('');

  statusEl.textContent = Status.AddTransaction;
  toggleState(true, false);

  function addTransaction() {
    gont_blockchain.createTransaction({
    ...
    pendingTransactionsEl.textContent = gont_blockchain.pendingTransactions.map(t =>
  ...
  async function mineBlock() {
    ...
    await gont_blockchain.minePendingTransactions();
